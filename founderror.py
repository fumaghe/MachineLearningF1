...
def create_features_and_labels(data, target_col):
    try:
        ...
        print(f"Errore nella creazione delle features e labels: {e}")
...
def train_and_predict(train_data, test_data, target_col):
    try:
        ...
        if X_train.empty or y_train.empty or X_test.empty:
            raise ValueError(f"Dati di training o test vuoti per il target: {target_col}")
        ...
        print(f"Errore durante l'addestramento o la predizione per il target {target_col}: {e}")
        return np.array([])
